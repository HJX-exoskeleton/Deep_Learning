...
def main():
    ...
    for epoch in range(n_epochs):
        ...
        # 在每个epoch结束时，生成并存储图像而不是显示它们
        with torch.no_grad():
            z = torch.randn(num_samples, latent_dim).to(device)
            sample = vae.decoder(z).cpu()
            images = sample.view(num_samples, input_dim, input_dim).numpy()  # 确保这里的维度与你的数据集匹配
            generated_images.append(images)


    # 定义保存图像的路径
    save_path = 'D:/Python_file/Deep_Learning/pycharm_pytorch/25-图像生成/Generated_Images/vae_data'
    if not os.path.exists(save_path):
        os.makedirs(save_path)

    # 在所有训练完成后显示和保存图像
    for epoch, images in enumerate(generated_images):
        fig, axs = plt.subplots(1, num_samples, figsize=(10, 10))
    ...
    # 保存图像
    for i, img in enumerate(images):
        plt.imsave(os.path.join(save_path, f'epoch{epoch}_sample{i}.png'), img, cmap='gray')
